...
export default class GooglePubSub implements PubSubEngine {
	...
	private async getSubscription(topicName, subName, filterOptions?) {
		...
		if (exists) {
			...
			const [newSub] = await this.pubSubClient
				.topic(topicName)
				.createSubscription(`${subName}`, filterOptions);
			return newSub;
		}
	}
	...
	public async subscribe(topicName, onMessage, options?) {
		...
		const googleSubAndClientIds: any =
			this.googleSubName2GoogleSubAndClientIds[subName] || {};
	...
	private getMessageHandler(subName) {
		...
		async function handleMessage(message) {
			...
			const { ids = [] } =
				engine.googleSubName2GoogleSubAndClientIds[subName] || {};
			ids.forEach((id) => {
				const [, onMessage] =
					engine.clientId2GoogleSubNameAndClientCallback[id];
				onMessage(res);
			});
		}
	...
	public filterOptions: FilterOptions; // Todo: type
